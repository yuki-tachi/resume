...
* プロジェクト詳細
    ...
    * バックエンドがAWS lambdaとAPI Gatewayを使ったサーバレス構成。フレームワークにChaliceを使用
    * フロントはreact+typescript
    * Amazon Cognitoを使ったログイン機能の実装
    * paypalのAPIを使用した購入処理の実装
    * セキュリティの高いデータにアクセスする際のスマホによる２要素認証を実現する外部システムの組み込み
    * ヨーロッパ語圏の多言語化対応

### 自動車事故対策機構ステアリングコントローラのWebアプリ
ゲーム用のハンドルコントローラーのSDKを使い、ブラウザ用にビルドしたゲーム内の車両を操作できるかの技術検証
...
### React
* 業務で約2年ほど使用
* v18.0でReactHookFormやMUIなどの外部ライブラリ組み込み、外部API呼び出し処理のカスタムフック化、メモ化などの最適化などの実装を行える
* Next.jsに関しては業務経験は無いがある程度理解している

### ASP.NET core
* 業務で1年ほど使用
* 環境構築からO/Rマッパーを使用してDBとやりとり、LINQを使った値の操作、ミドルウェアを使った例外やログのハンドリングなどの実装を行える
